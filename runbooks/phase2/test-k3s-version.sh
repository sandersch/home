...
mock_k3s_version="v1.36.4+k3s1"
if (assert_k3s_version >/dev/null 2>&1); then
  die "version assertion accepted an unexpected k3s release"
fi

grep -Fq 'use runbooks/phase2/07-upgrade-k3s.sh for an attended upgrade' "$script_dir/lib.sh" \
  || die "bootstrap version mismatch does not direct the operator to the upgrade runbook"

ok "k3s bootstrap pin and active-version guard passed"
