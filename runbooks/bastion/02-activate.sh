...
rcctl disable dhcpleased resolvd rad slaacd
...
rcctl enable sshd ntpd
rcctl start sshd
rcctl restart ntpd

# Verify the hardened daemon is bound only to its final operator-side address
# before making TCP/22 reachable. A failure above or here leaves deny-all active.
