...
yq -y -i '.spec.template.spec.containers[0].command = ["/bin/bash", "-c", "/guards/assert-backups-mount.sh && exec /scripts/validate-vault-photos.sh"] | .spec.template.spec.containers[0].env += [{"name":"PHOTO_SNAPSHOT","value":env.VAULT_SNAPSHOT}] | .spec.template.spec.containers[0].securityContext.capabilities.add += ["CHOWN"]' "$manifest"
