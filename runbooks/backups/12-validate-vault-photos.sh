...
yq -y -i --arg sid "$VAULT_SNAPSHOT" '.spec.template.spec.initContainers = [] | .spec.template.spec.volumes |= map(select(.name != "ingestion-script" and .name != "frigate-exports")) | .spec.template.spec.containers[0].command = ["/bin/bash", "-c", "/guards/assert-backups-mount.sh && exec /scripts/validate-vault-photos.sh"] | .spec.template.spec.containers[0].env += [{"name":"PHOTO_SNAPSHOT","value":$sid}] | .spec.template.spec.containers[0].securityContext.capabilities.add += ["CHOWN", "FOWNER"]' "$manifest"
