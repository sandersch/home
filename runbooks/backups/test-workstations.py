...
class ScopeTests(unittest.TestCase):
    ...
    def test_dropbox_alias_cannot_escape_or_use_symlink_ancestors(self):
        records = {'Dropbox': {'type': 'symlink'}, 'Library': {'type': 'dir'},
                   'Library/CloudStorage': {'type': 'dir'},
                   'Library/CloudStorage/Dropbox': {'type': 'dir'},
                   'Library/CloudStorage/Dropbox/ccs.kdbx': {'type': 'file', 'size': 102400}}
        self.assertEqual(client.database_path(records, '/Users/test'),
                         'Library/CloudStorage/Dropbox/ccs.kdbx')
        records['Library'] = {'type': 'symlink'}
        with self.assertRaisesRegex(ValueError, 'ancestors'):
            client.database_path(records, '/Users/test')

    def test_symlink_target_drift_is_not_global(self):
        expected = {'link': {'type': 'symlink', 'linktarget': 'before'}}
        actual = {'link': {'type': 'symlink'}}
        self.assertEqual(client.drift(expected, actual), [])
        self.assertEqual(client.drift(expected, {'link': {'type': 'file', 'size': 1}}), ['link'])
