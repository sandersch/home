...
|---|---|---|---|
| NIC1 (`lan0`) | Primary LAN | `172.17.1.5/24` | Internet, NAS NFS, host SSH, k3s API |
| NIC2 (`cam0`) | Camera segment | `192.168.104.1/24` | Isolated; serves DHCP; Frigate only |
...
**Camera isolation (nftables, host-level).** The `camera_isolation` table hooks two chains, both `policy accept` with explicit drops. The **forward** chain handles *routed* traffic: `iifname "cam0" drop` blocks camera-initiated connections (camera→internet, camera→LAN) and `oifname "cam0" drop` blocks forwarded LAN→camera traffic (access to camera web UIs goes via the host, e.g. SSH port-forward). The **input** chain handles traffic the camera sends *to the host itself* — which the forward chain never sees, since locally-delivered packets hit the input hook. Without it, host services bound to `0.0.0.0` (k3s `:6443`, kubelet `:10250`, SSH, and every `hostNetwork` pod: Frigate, Plex, Home Assistant) would be reachable from a compromised camera on `192.168.104.1`. The input chain allows only `ct state established,related` (RTSP replies — Frigate initiates, so this is the return path), DHCP (`udp dport 67`), NTP (`udp dport 123`), and IPv4 ICMP for diagnostics, then drops the rest from `cam0`. The drops are logged (rate-limited `cam-drop-*` prefixes) so a compromised camera's blocked traffic is visible in the journal rather than silently discarded. The segment is IPv4-only (IPv6 disabled on `cam0`); `ip protocol icmp` matches IPv4 ICMP only, and any ICMPv6 falls to the drop. `policy accept` is intentional — `policy drop` would break k3s pod networking, because every hook chain (these and k3s's own iptables-nft chains) is evaluated independently for each packet, and a drop verdict in any chain is final even when another chain accepts. Frigate runs with `hostNetwork: true`, so its RTSP connections to cameras originate from the host's NIC2 address (`192.168.104.1`) and never pass through the forward chain. Validate before cameras go live: from the camera segment, ping `8.8.8.8` and a LAN host (`172.17.1.5`) must both fail, and `nc -vz 192.168.104.1 22` (a port that is actually listening) must fail — ICMP to the host still succeeds.
