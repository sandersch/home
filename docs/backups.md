...
copy/prune/verification jobs, and attended offline runbook — must run the
same preflight before opening a repository or destination credential. Textfile metrics are
not on this list: they are written to `/var/lib/node-exporter/textfile/` on the root
filesystem rather than to the array, for the reasons in
[The metric surface](#the-metric-surface-for-job-derived-alerts). Hold, ledger, and
baseline writes under `/mnt/backups/.control/` are covered like any other array write.
...
- fail before creating a directory, writing a hold or ledger entry, or opening a repository
  on any mismatch. A scheduled Job fails non-zero; rest-server has both a startup/init gate and a
...
makes overlap safe, not free. The recurring vault copy and prune jobs use a bounded three-hour
`--retry-lock`; the two-hour vault backup Job uses a 90-minute bound, and passes that bound to
shared snapshot validation. A legitimate overlap therefore waits without allowing a backup
Job to outlive its deadline. Workstation prune scheduling remains separate from this policy.
...
`/var/lib/node-exporter/textfile/` into it. Trusted jobs and attended runbooks write one
...
**These files live on the root filesystem, deliberately not beside `.control/` on the
array.** An earlier draft put them under `/mnt/backups/.control/metrics/`, which is the
same convenience that makes it easy to miss: node-exporter would then hold a
`hostPath` on the backup array, and the fail-closed rules above make that hostPath
*unresolvable* whenever the array is down — `type: Directory` fails to mount, and
`DirectoryOrCreate` returns `EPERM` against the immutable `/mnt/backups` mountpoint. The
pod would not start, and node-exporter is the sole source of
`BulkStorageMountSetIncomplete`, `BackupsVolumeFillingUp`, and `VaultLocked`. An array
failure would take out precisely the alerts that report an array failure. The properties
are also inverted from the control state's: `.prom` files are transient, regenerated by
the next run of whatever wrote them, and their absence is *supposed* to be loud — every
rule in § 9 carries `or absent(...)` for exactly that. Nothing about them wants array
durability.

The consequence is that metric writing is no longer a caller of the `/mnt/backups`
preflight at all: a job asserts the mount before opening a repository, and writes its
`.prom` file afterward on a filesystem that is always present. `/var` is its own LV
(`docs/architecture.md`), so the surface survives a full `hoardvg` outage and reports it.

**What stays under `/mnt/backups/.control/` stays there for the opposite reason.** Holds,
validation ledgers, and `baseline.json` are *authority* over destructive operations, and
they are deliberately co-fated with the repositories they gate: if the array is gone, their
absence is moot because the repos are gone too. Putting them on the root NVMe instead would
mean a root-disk rebuild left healthy repositories with no validation state — prune would
fail closed, correctly, but recovering from an event that never touched the backups would
require attended revalidation of every repo. Keep the split: authority on the array with
the repositories, evidence about it on the root filesystem with the collector.
...
- **Monitoring survives the array.** The metric surface is on the root filesystem
  specifically so an array failure is *reported* rather than silenced, and that only holds
  if node-exporter never depends on `/mnt/backups`. Inspect the rendered node-exporter
  DaemonSet and confirm its only textfile `hostPath` is `/var/lib/node-exporter/textfile`
  and that no volume references the backup array. Then, on a host where the array is
  legitimately absent — a fixture or a rebuild, never by unmounting the production LV —
  confirm node-exporter starts, `node_filesystem_*` is served, and
  `BulkStorageMountSetIncomplete` and `BackupsVolumeFillingUp` evaluate as intended. A
  node-exporter pod stuck in `FailedMount` here means the textfile directory drifted back
  onto the array, which is a regression that disables the alerts reporting it.
...
| `m5c` workstation local + B2 restore | 2026-09-14 (partial) | NAS metadata and representative-file verification passed for `52e7cfdf`: 361,195 nodes, FinderInfo/ResourceFork, and selected symlink target. Full NAS content verification evidence, independent B2 recovery, and manual KDBX openings remain pending; see the [verification record](../runbooks/backups/workstations.md#2026-09-14-m5c-nas-restore-verification). |
