...
| Item | When to do it |
|---|---|
| **Whole-estate/off-site backup plan** | **Partially complete.** Vault B2 enrollment, seed, on-host and card-only off-host restores, authorization separation, and repository verification passed on 2026-09-07. Flux applied activation commit `eb02a61`; copy/prune CronJobs are live with `SUSPEND=false`, and the first copy run completed successfully. Workstation clients are active. [Offline SSD tooling](../runbooks/backups/offline-ssd.md) and staged warning rules are implemented; A/B physical enrollment, independent recovery and alert activation remain pending. After both enrollments, rotate A in Q2/Q4 and B in Q1/Q3, starting A in Q4 2026. Mail archival remains draft. Do not describe the RAID array itself as a backup. |
