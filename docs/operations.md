...
independent card-only restore from `ryze`, and appstate-key denial from the vault bucket
passed on 2026-09-07; recurring B2 copy/prune remain suspended pending the remaining final
activation gates.
...
| Item | When to do it |
|---|---|
| **Whole-estate/off-site backup plan** | **Still open.** The encrypted vault’s local photo pipeline is deployed and its full restore drill passed, but vault B2 replication, workstation backups, and offline copies remain draft. Complete those destinations, retention, monitoring, and representative restore drills before treating the broader plan as implemented; do not describe the RAID array itself as a backup. |
