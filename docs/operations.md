...
| Item | When to do it |
|---|---|
| **Bulk-storage backup plan** | **Still open.** A proposed design now exists in [backups.md](./backups.md) — but it is a **draft and nothing in it is deployed**, so this item stays here until it is built and drilled. Replace the current ad hoc external-drive copies with a documented, repeatable backup policy for irreplaceable content on `/mnt/media` (including personal pictures) and any other selected bulk-array paths. Define the destination, cadence, retention, monitoring, and representative restore drills; until then, do not describe the RAID array itself as a backup. Retire this row only once the drills in [backups.md → Verification](./backups.md#verification-proposed) have actually passed. |
